...
\step{Add recorded impulses (the ``poke'' channel)}
At 1--3 random recorded frames per trajectory, apply \texttt{p.applyExternalForce} at a random point on a random body's surface with random direction and magnitude (calibrate magnitudes so pokes visibly move objects without launching them off-screen). Save, per impulse: the frame index, body id, application point, and force vector. This is the action channel: at inference time a mouse drag will be encoded the same way.
\begin{trap}
...
\step{The integrator}
In \texttt{model/integrator.py}: denormalize predictions, add gravity, semi-implicit Euler per body, rotation update via rotation-vector exponential, then particles from body poses. Keep it a pure function of (state, prediction) $\rightarrow$ state; Phase 7 reimplements it in TypeScript and purity makes the two easy to compare.
...
\step{Mouse-to-impulse}
On drag: raycast into the scene, hit-test body bounding boxes, convert drag direction and length into an impulse vector (capped at the training distribution's max), send it. Server encodes it into the action feature $\mathbf{a}^{\text{ext}}$ on the hit body's particles for $K{=}3$ steps with Gaussian falloff from the hit point, exactly matching how impulses were encoded in training data.
\begin{trap}
