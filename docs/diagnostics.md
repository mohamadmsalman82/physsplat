...
- **free flight**: a body with no edge to another body and no particle within the contact radius of the floor gets a zero residual; only gravity and the applied force act on it. Without this, a pencil released in mid-air hovered (the network never saw a motionless unsupported body in training). The same applies to a body that touches things but has nothing below it (only pencils on its back, a neighbour beside it, or the table a few millimetres down): it falls, and they come down with it. Support is followed transitively: a body counts as held up only if the chain of bodies under it reaches the table or the held pencil, so a tangle let go in mid-air falls as a tangle (`web/test/ring.mjs`). One exception: a body still descending into the floor zone faster than 3 cm/s is landing, and the model's contact response lands it.
- **pivot**: a body whose centre of mass is not over its support (more than 3 mm outside the support polygon, with hysteresis back to the 2 mm balance tolerance) is integrated as a pendulum about the hinge, the nearest point of the support polygon's boundary (an edge between two contacts, or a lone contact): gravity's torque over the inertia about the hinge, model residual dropped, centre of mass moving with omega x r; velocities are cut to 0.2 when the swing lands on a new support. Without this a pencil that landed on its end was held 9-14 degrees up with the other end in the air, and pencils balanced on their tips (the round-1 tester's complaint). The hinge has to be a real contact: within 1.5 mm of the table or a neighbour (3 mm once it has touched), with a normal at least 30 degrees above horizontal, and no air straight beneath the body. Floor contact comes from the analytic surface, not the particles. Until it was, a pencil 4 mm above the desk and touching nothing had a support polygon, tipped about a point in mid-air, and skated there because the pendulum integration gives gravity no vote (`web/test/float.mjs`, the floating a player photographed).
- **the table cannot throw or pitch a pencil**: for a body whose only contact is the table, the step may not leave it moving upward faster than free fall or a bounce at restitution 0.2 would; the model's response to a fast body a few millimetres up was 14 to 33 m/s^2 and launched pencils 10 cm on end. A pencil lying along the table (real contact over 20 mm or more of its length) gets no pitch torque from the model either; roll and yaw stay. And a point that meets the table stops: the ground guard and the swept table check remove the spin that is driving the lowest point down, instead of only lifting the body out.
...
- **settle**: a supported body that has been slow (under 3 cm/s, 0.6 rad/s) for 15 steps is held exactly still (pose restored) until something acts on it; slow motion on a support is damped by half each step first, because the model's contact response rings for a second after a landing. Zeroing velocity alone left a slow sideways creep driven by the guards. At the moment a body settles, a gap of up to 6 mm to the floor or to the nearest capsule is closed once, because the model's contact response equilibrates 2-5 mm above whatever it landed on. "Supported" here is the balance verdict on real contacts only, judged before the step.
- **seat**: every unheld body with air straight beneath it, up to 30 mm, is lowered a millimetre a step until something is really under it, whatever its speed. The model senses contact through a 6 mm radius and holds a body several millimetres short of its support; a body can also sleep only once that gap is closed.
- **angular contact fade**: contact torque must vanish as a body separates. The model's does not, so a pencil lifted off a pile kept receiving hundreds of rad/s^2 across millimetres of gap and tumbled 50-78 degrees although a grab at its centre of mass applies no torque. The angular residual now fades linearly to zero at the contact radius, where free flight takes over; the linear part is untouched because it holds the pile up. Scored and kept: composite 63.6 against 62.3 for free flight alone, with the best translation error, axis error and surface penetration of any configuration tried.
- **a pinch, not a point**: a held body gets angular damping, its learned angular residual is two-tap averaged and capped at 1.5 rad/s^2, and its learned linear residual may only push it away from what it touches (a contact cannot pull; the model answered "down, into the neighbour" for a pencil held still on one and the guard fought it every step). A single-point spring resists no rotation at all, so any spin picked up while separating from the pile persisted; fingers do not behave that way. This lives in the interaction model, not the physics.
- **no free energy, for bodies sitting still**: nothing lifts a resting pencil, so with no applied force and no moving neighbour its mechanical energy cannot rise. Each step is trial-integrated, and a quiescent body (under 5 cm/s and 1 rad/s, not held, not next to something moving) whose energy would rise gets its residual scaled down until it does not (`guard.energyScale`, `totals.energyGain_uJ`). Without this the top pencil of IMG_8596 reared up to 89 degrees on its own, twice in ten seconds, and balanced on its end. The restriction to quiescent bodies is not a detail: an impact is a legitimate energy spike, and the first version policed the whole scene every step, cutting contact impulses and dropping the synthetic scorecard from 62.3 to 42.4 (stability 0.83 to 0.38, support-removal Jaccard 0.60 to 0.27).
...
Each rule sets a flag in the per-step `guard` record (`freeFlight`, `pivot`, `settled`, `settleVia`, `floorCap`, `floorTorque`, `groundSpin`, `seat`) so a report can always say whether the model or a rule produced a motion.
...
of rise, 0.0 degrees of tilt, 0.0 mm of drift and no sinking.

`web/test/float.mjs` plays each scene for thirty random gestures (lifts,
carries, slides, presses into the desk, high drops, flicks, holds) and after
every step asks whether each unheld pencil rests on a chain of contacts that
reaches the table; a pencil in the air for half a second with its height
unchanged fails the scene. `web/test/ring.mjs` builds three and four pencils
woven so each rests on the next, hanging in mid-air, and checks that they
come down. `web/test/hover.mjs` places a pencil 3 to 5 mm above the desk
and above a crossing pencil, inside the model's contact band, and checks
that it reaches contact within half a second and stays there (it does, on
