...
- **a pinch, not a point**: a held body gets angular damping. A single-point spring resists no rotation at all, so any spin picked up while separating from the pile persisted; fingers do not behave that way. This lives in the interaction model, not the physics.
