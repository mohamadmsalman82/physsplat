...
A learned simulator asked to run far outside its training distribution --- photo-reconstructed pencils rather than PyBullet capsules --- asserts things that are not physics. Three invariants are therefore stated explicitly in the runtime, and every intervention is recorded per step so a reader can always separate the model's answer from the correction.
...
\emph{No free energy.} Contact forces from static things cannot add mechanical energy to a scene, and an applied force can add only the work it does. Each step is trial-integrated; if the residual would raise $\sum_b \tfrac{1}{2}m_b\|\vv_b\|^2 + \tfrac{1}{2}\boldsymbol\omega_b^{\!\top}\mathbf{I}_b\boldsymbol\omega_b + m_b g z_b$ by more than $\mathbf{f}\cdot\vv_{\text{grab}}\,\Delta t$ (plus $0.5\,\mu$J of slack for pushing out of overlaps), the residual is scaled down by bisection until it does not. Without this rule a pencil at rest rotated up to $89^\circ$ on its own and balanced on its end.
...
Two smaller cleanups sit alongside them: residual capsule and floor overlap is removed geometrically, and a supported body that has been slow for fifteen steps is held exactly still, since the model's contact response rings for about a second after every landing. All five are demo-only by default; \texttt{scripts/evaluate.py} exposes the two physical rules as flags so their effect on the scorecard is measured rather than assumed.

\subsection{Diagnostics}
\label{sec:diagnostics}
The demo carries a diagnostic layer that records, every step: per-body pose and velocities, lowest surface point, axis elevation, capsule contacts with signed gaps, which bodies support which, penetration depth, resting state and drift since rest, kinetic energy, the model's predicted acceleration, and each rule's intervention. Ten detectors (explosion, sinking, floating, tip balance, tilt hold, unbalanced rest, rearing, creep, penetration, jitter, spontaneous motion) open and close logged episodes with peaks. On top of that sit scripted probes --- rest, lift, grab at an end, poke, flick, pull the load-bearing body, drop --- which drive the same code path as the mouse and return measurements: cursor tracking error, fitted free-fall acceleration, contact onset, impact speed against the analytic value, bounces, time to rest, and neighbour displacement. The point is that a tester, human or automated, reads what the pencils did instead of inferring it from pixels; every defect listed in Section~\ref{sec:invariants} was found and confirmed this way.
